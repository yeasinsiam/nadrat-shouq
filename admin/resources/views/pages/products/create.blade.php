@extends('layout.main')
@section('meta-title', 'Create Product')
    ...
    <form action="{{ route('products.store') }}" enctype="multipart/form-data" method="POST">
        ...
        <x-layout.content.__row class="mb-5">
            <x-layout.content.__col lg="7">
                <x-layout.content.__breadcrumb :items="[['title' => 'Product List', 'link' => route('products.index')], ['title' => 'Add Product']]" />
        ...
        <x-layout.content.__row gap='4' class="justify-content-center">
                                ...
                                <x-layout.content.__row gap='3'>
                                    ...
                                    <x-layout.content.__col lg="12">
                                        <x-forms.__select-field label="Select Category" name="product_category_id"
                                            placeholder="Select Category" :options="$productCategories->toArray()" required :error="$errors->has('product_category_id')"
                                            :helpText="$errors->has('product_category_id')
                                    ...
                                    {{-- Fabric --}}
                                    <x-layout.content.__col lg="6">
                                        <x-forms.__text-field name="fabric" label="Fabric" placeholder="Enter Fabric"
                                            :error="$errors->has('fabric')" :helpText="$errors->has('fabric') ? $errors->first('fabric') : ''" />
                                    </x-layout.content.__col>
                                    {{-- Material --}}
                                    <x-layout.content.__col lg="6">
                                        <x-forms.__text-field name="material" label="Material" placeholder="Enter Material"
                                            :error="$errors->has('material')" :helpText="$errors->has('material') ? $errors->first('material') : ''" />
                                    </x-layout.content.__col>
                                    {{-- Measurement --}}
                                    <x-layout.content.__col lg="6">
                                        <x-forms.__text-field name="measurement" label="Measurement"
                                            placeholder="Enter Measurement" :error="$errors->has('measurement')" :helpText="$errors->has('measurement') ? $errors->first('measurement') : ''" />
                                    </x-layout.content.__col>
                                    {{-- Warranty --}}
                                    <x-layout.content.__col lg="6">
                                        <x-forms.__text-field name="warranty" label="Warranty" placeholder="Enter Warranty"
                                            :error="$errors->has('warranty')" :helpText="$errors->has('warranty') ? $errors->first('warranty') : ''" />
                                    </x-layout.content.__col>
                                    {{-- Description --}}
                                    <x-layout.content.__col lg="12">
                                        <x-forms.__textarea-editor-field name="description" label="Description"
                                            placeholder="Enter Description" :error="$errors->has('description')" :helpText="$errors->has('description') ? $errors->first('description') : ''" />
                                    </x-layout.content.__col>
            ...
            <x-layout.content.__col lg="6">
                <x-layout.content.__card>
                    ...
                    <x-slot:body>
                        <x-forms.__rich-file-field {{-- label="Gallery" --}} multiple name="gallery" accept=".jpg,.jpeg"
                            :previewFileWidth="150" :previewFileHeight="150" :fileItemMinWidth="500" :error="$errors->has('gallery')"
