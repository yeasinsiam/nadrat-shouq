@extends('layout.main')
@section('meta-title', 'Edit Product Categories')
    ...
    <x-layout.content.__row gap='4' class="justify-content-center">
        <x-layout.content.__col lg="7">
            <x-layout.content.__card>
                <x-slot:body>
                    <form action="{{ route('product-categories.update', $productCategory->id) }}"
                        ...
                        <x-layout.content.__row gap='2'>
                            <x-layout.content.__col lg="6">
                                <x-forms.__text-field name="name" label="Name" placeholder="Enter Name"
                                    inputType="text" :value="old('name', $productCategory->name)" required :error="$errors->has('name')" :helpText="$errors->has('name') ? $errors->first('name') : ''" />
                            </x-layout.content.__col>
                            <x-layout.content.__col lg="6">
                                <x-forms.__text-field name="slug" :value="old('slug', $productCategory->slug)" label="Slug (Optional)"
                                    placeholder="Enter Slug" :error="$errors->has('slug')" :helpText="$errors->has('slug') ? $errors->first('slug') : ''" />
                            </x-layout.content.__col>
                            <x-layout.content.__col lg="12">

                                <x-forms.__rich-file-field label="Icon" name="icon" :media="$productCategory->icon"
                                    accept=".png,.svg" :previewFileWidth="39" :previewFileHeight="39" :fileItemMinWidth="400"
